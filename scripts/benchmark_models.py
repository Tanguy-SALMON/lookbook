...
class ModelBenchmark:
    ...
    def _get_system_metrics(self) -> Tuple[float, float, Optional[float], Optional[float]]:
        ...
        try:
            async with httpx.AsyncClient(timeout=self.config.timeout) as client:
                ...
                if response.status_code == 200:
                    ...
                    tokens = len(response_text.split()) * 1.3  # Average 1.3 tokens per word
                    tokens_per_second = tokens / elapsed_time if elapsed_time > 0 else 0

                    return response_text, elapsed_time, True, None
                else:
                    elapsed_time = time.time() - start_time
                    return "", elapsed_time, False, f"HTTP {response.status_code}: {response.text}"
    ...
    def _evaluate_response_quality(self, prompt: str, response: str) -> float:
        ...
        fashion_keywords = ["wear", "outfit", "clothes", "fashion", "style", "look", "fit"]
        ...
        if len(sentences) >= 2:
            score += 0.2
        if any(word in response_lower for word in ["recommend", "suggest", "consider", "try"]):
            score += 0.1
        ...
        if not response.strip().count('  ') > 2:  # Not too many double spaces
            score += 0.05
        if len([c for c in response if c.isupper()]) / len(response) < 0.3:  # Not all caps
            score += 0.05
    ...
    async def run_single_benchmark(self, prompt: str) -> BenchmarkResult:
        ...
        final_memory, final_cpu, final_gpu, final_gpu_memory = self._get_system_metrics()
        ...
        avg_gpu_memory = (gpu_memory + final_gpu_memory) / 2 if gpu_memory is not None else None
        ...
        quality_score = self._evaluate_response_quality(prompt, response) if success else 0.0
    ...
    async def run_full_benchmark(self) -> List[BenchmarkResult]:
        ...
        print(f"Running {self.config.repeat_count} iterations for each of {len(self.config.test_prompts)} prompts")
    ...
    def _calculate_performance_metrics(self, results: List[BenchmarkResult]) -> Dict[str, Any]:
        ...
        return {
            ...
            "tokens_per_second": {
                ...
                "std": statistics.stdev(tokens_per_second) if len(tokens_per_second) > 1 else 0,
    ...
    def _calculate_quality_metrics(self, results: List[BenchmarkResult]) -> Dict[str, Any]:
    ...
    def _calculate_variability_metrics(self, prompt_results: Dict[str, List[BenchmarkResult]]) -> Dict[str, Any]:
        ...
        # Overall variability
        all_response_times = [r.response_time for r in prompt_results.values() for r in r]
        all_quality_scores = [r.response_quality_score for r in prompt_results.values() for r in r]

        return {
            "overall_response_time_std": statistics.stdev(all_response_times) if len(all_response_times) > 1 else 0,
            "overall_quality_score_std": statistics.stdev(all_quality_scores) if len(all_quality_scores) > 1 else 0,
            "prompt_variability": prompt_variability
        }

    def _analyze_by_prompt(self, prompt_results: Dict[str, List[BenchmarkResult]]) -> Dict[str, Any]:
    ...
    def save_results(self, output_dir: str = "benchmark_results") -> str:
        ...
        results_file = os.path.join(output_dir, f"{model_name_safe}_results_{timestamp}.json")
        with open(results_file, 'w') as f:
            json.dump([r.__dict__ for r in self.results], f, indent=2)

        # Save analysis
        analysis = self.analyze_results()
        analysis_file = os.path.join(output_dir, f"{model_name_safe}_analysis_{timestamp}.json")
        with open(analysis_file, 'w') as f:
            json.dump(analysis, f, indent=2)

        # Save summary
        summary_file = os.path.join(output_dir, f"{model_name_safe}_summary_{timestamp}.txt")
...
Model: {self.config.model_name}
Timestamp: {datetime.now().isoformat()}
System: {self.system_info['cpu_count']} cores, {self.system_info['total_memory_gb']:.1f}GB RAM

PERFORMANCE METRICS
-------------------
Response Time:
  Mean: {analysis['performance_metrics']['response_time']['mean']:.2f}s
  Median: {analysis['performance_metrics']['response_time']['median']:.2f}s
  Std Dev: {analysis['performance_metrics']['response_time']['std']:.2f}s
  P95: {analysis['performance_metrics']['response_time']['p95']:.2f}s
  P99: {analysis['performance_metrics']['response_time']['p99']:.2f}s

Throughput:
  Mean Tokens/sec: {analysis['performance_metrics']['tokens_per_second']['mean']:.1f}
  Median Tokens/sec: {analysis['performance_metrics']['tokens_per_second']['median']:.1f}

Resource Usage:
  Memory (MB): {analysis['performance_metrics']['memory_usage_mb']['mean']:.1f} ± {analysis['performance_metrics']['memory_usage_mb']['std']:.1f}
  CPU (%): {analysis['performance_metrics']['cpu_usage_percent']['mean']:.1f} ± {analysis['performance_metrics']['cpu_usage_percent']['std']:.1f}

QUALITY METRICS
-------------------
Quality Score: {analysis['quality_metrics']['quality_score']['mean']:.3f} ± {analysis['quality_metrics']['quality_score']['std']:.3f}
Response Length: {analysis['quality_metrics']['response_length']['mean']:.0f} ± {analysis['quality_metrics']['response_length']['std']:.0f} chars

VARIABILITY METRICS
-------------------
Overall Response Time Std: {analysis['variability_metrics']['overall_response_time_std']:.2f}s
Overall Quality Score Std: {analysis['variability_metrics']['overall_quality_score_std']:.3f}
...
"""
        ...
        sorted_prompts = sorted(prompt_analysis.items(), key=lambda x: x[1]['avg_quality_score'], reverse=True)
...
async def generate_comparison_report(results_files: Dict[str, str]):
    ...
    for model, results_file in results_files.items():
        ...
        if os.path.exists(analysis_file):
            ...
            if successful_results:
                ...
                quality_scores = [r['response_quality_score'] for r in successful_results]
    ...
    print("-" * 80)
    print(f"{'Model':<15} {'Mean Time (s)':<12} {'Median Time (s)':<15} {'Quality Score':<12} {'Success Rate':<12}")
    print("-" * 80)
    ...
    for model, data in comparison_data.items():
        ...
        print(f"{model:<15} {perf['mean']:<12.2f} {perf['median']:<15.2f} {quality['mean']:<12.3f} {success_rate:<12.1f}")

    # Find best model
    best_performance = min(comparison_data.items(), key=lambda x: x[1]['performance_metrics']['response_time']['mean'])
    best_quality = max(comparison_data.items(), key=lambda x: x[1]['quality_metrics']['quality_score']['mean'])

    print(f"\nRECOMMENDATIONS:")
    print(f"Fastest Model: {best_performance[0]} ({best_performance[1]['performance_metrics']['response_time']['mean']:.2f}s)")
    print(f"Best Quality: {best_quality[0]} ({best_quality[1]['quality_metrics']['quality_score']['mean']:.3f})")
...
async def main():
    ...
    parser = argparse.ArgumentParser(description="Benchmark LLM models for chatbot applications")
    parser.add_argument("--models", nargs="+", default=["qwen3:4b", "qwen3"],
    ...
    if len(args.models) > 1:
        results_files = {model: f"{args.output}/{model.replace(':', '_').replace('.', '_')}_results_*.json"
                        for model in args.models}
