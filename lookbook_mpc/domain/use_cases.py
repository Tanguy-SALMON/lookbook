...
class ChatTurn(UseCase):
    ...
    async def execute(self, request: ChatRequest) -> ChatResponse:
        """
        Execute chat turn processing with AI-powered fashion recommendations.

        This is the main orchestration method that coordinates the entire recommendation pipeline:
        1. Parse user intent using LLM (natural language → structured data)
        2. Determine if outfit recommendations are needed
        3. Generate product recommendations using Smart Recommender
        4. Format response for frontend consumption
        5. Return natural language response + outfit suggestions

        Example Input: ChatRequest(message="I go to dance", session_id="123")
        Example Output: ChatResponse(
            replies=[{"type": "recommendations", "message": "Perfect! I'll help you...", "outfits": 2}],
            outfits=[{"title": "Party Ready Look", "items": [...], "total_price": 89.0}],
            session_id="123"
        )
        ...
        Returns:
            ChatResponse with natural LLM replies and optional outfit recommendations
        """
        try:
            # Step 1: Parse user intent from message using LLM
            # Input: "I go to dance" → Output: {"activity": "dancing", "occasion": "party", ...}
            intent = await self.intent_parser.parse_intent(request.message)

            # Extract the natural response generated by LLM for user-friendly reply
            natural_response = intent.get(
                "natural_response", "I'm here to help you find the perfect outfit!"
            )

            # Step 2: Determine if this is a recommendation request or just conversation
            # Check both the parsed intent and presence of fashion-related keywords
            is_recommendation_request = intent.get(
                "intent"
            ) == "recommend_outfits" and any(
            ...
            # Step 3: Try to generate real outfit recommendations using smart recommender
            # This calls the main recommendation pipeline: keywords → search → assembly
            try:
                ...
                if outfit_recommendations and is_recommendation_request:
                    # Step 4: Convert smart recommender output to frontend-compatible format
                    # Transform internal format to what the React frontend expects
                    formatted_outfits = []
                    total_outfits = len(outfit_recommendations)

                    for outfit in outfit_recommendations:
                        # Transform each outfit from internal format to frontend-compatible structure
                        # Internal: {"title": "...", "items": [...], "style_explanation": "..."}
                        # Frontend: {"title": "...", "items": [...], "rationale": "...", "score": 0.85}
                        formatted_outfit = {
                            "title": outfit["title"],
                            "items": outfit[
                                "items"
                    ...
                    # Step 5: Enhance the natural response with actual findings
                    # Combine LLM's natural language with concrete results
                    ...
                    replies = [
                        {
                            "type": "recommendations",  # Tells frontend to show recommendation UI
                            "message": enhanced_response,
                            "outfits": total_outfits,
                        }
                    ]

                    # Return formatted outfits for frontend display
                    outfits = formatted_outfits
                else:
                    # No outfits found or not a recommendation request - just conversation
                    # Use the natural response from LLM for friendly chat interaction
                    replies = [
                        {
                            "type": "assistant",  # Tells frontend to show as chat message
                ...
                # Fallback: If recommendation fails, still provide helpful LLM response
                # This ensures users always get a response even if product search fails
                replies = [
                    {
                        "type": "assistant",
                        "message": natural_response,
            ...
            # replies: natural language responses with type indicators
            # outfits: complete outfit data for product display (null if no recommendations)
            return ChatResponse(
                session_id=request.session_id or str(uuid.uuid4()),
                replies=replies,
                outfits=outfits if outfits else None,  # null when no recommendations
                request_id=str(uuid.uuid4()),
            )

        except Exception as e:
            print(f"ChatTurn error: {e}")
            # Error fallback: Always return something helpful to maintain user experience
            # This prevents the frontend from showing error states for system failures
            return ChatResponse(
                session_id=request.session_id or str(uuid.uuid4()),
                replies=[
                    {
